...
async function setupGroupMessaging() {
  try {
    ...
    // Check and add groupId column to messages table
    const hasGroupId = await checkColumnExists('messages', 'groupId');
    if (!hasGroupId) {
      await sequelize.query(`
      ...
      await sequelize.query(`
        ALTER TABLE \`messages\`
        ADD CONSTRAINT \`messages_group_id_foreign\`
        FOREIGN KEY (\`groupId\`) 
    ...
    // Check and add imageUrl column to messages table
    const hasImageUrl = await checkColumnExists('messages', 'imageUrl');
    if (!hasImageUrl) {
      await sequelize.query(`
        ALTER TABLE \`messages\`
    ...
    if (hasGroupId) {
      try {
        await sequelize.query(`
          ...
          ON \`messages\` (\`groupId\`);
        `);
        console.log('Created index for messages groupId');
      } catch (indexError) {
        console.warn('Could not create messages groupId index:', indexError);
      }
    }
    ...
    try {
      ...
      `);
      console.log('Created indexes for group conversations');
    } catch (indexError) {
      console.warn('Could not create group conversations indexes:', indexError);
    }
    
    // Create group members indexes
    try {
      await sequelize.query(`
      ...
      `);
      console.log('Created indexes for group members');
    } catch (indexError) {
      console.warn('Could not create group members indexes:', indexError);
    }
