...
async function startServer() {
 ...
 async function migratePostTypes() {
  try {
    console.log("Starting social post types migration...");
    const { sequelize } = require('./models');
    
    // Check if the columns already exist to avoid errors
    const tableInfo = await sequelize.getQueryInterface().describeTable('social_posts');
    
    // Step 1: Add the postType column if it doesn't exist
    if (!tableInfo.postType) {
      console.log("Adding postType column...");
      await sequelize.query(`
        ALTER TABLE social_posts 
        ADD COLUMN postType ENUM('text', 'image', 'nowListening', 'multiple_images') 
        DEFAULT 'text' AFTER characterId
      `);
    }
    
    // Step 2: Add the mediaUrls column if it doesn't exist
    ...
    // Step 3: Add the song-related columns if they don't exist
    if (!tableInfo.songTitle) {
      console.log("Adding songTitle column...");
      await sequelize.query(`
        ALTER TABLE social_posts 
        ADD COLUMN songTitle VARCHAR(255) 
        NULL AFTER mediaUrls
      `);
    }
    
    if (!tableInfo.artistName) {
      console.log("Adding artistName column...");
      await sequelize.query(`
        ALTER TABLE social_posts 
        ADD COLUMN artistName VARCHAR(255) 
        NULL AFTER songTitle
      `);
    }
    
    if (!tableInfo.albumName) {
      console.log("Adding albumName column...");
      await sequelize.query(`
        ALTER TABLE social_posts 
        ADD COLUMN albumName VARCHAR(255) 
        NULL AFTER artistName
      `);
    }
    
    if (!tableInfo.albumCoverUrl) {
      console.log("Adding albumCoverUrl column...");
      await sequelize.query(`
        ALTER TABLE social_posts 
        ADD COLUMN albumCoverUrl VARCHAR(255) 
        NULL AFTER albumName
      `);
    }
    
    // Step 4: Update existing posts to have the 'text' type if they don't have one
    console.log("Updating existing posts to have 'text' type...");
    await sequelize.query(`
      UPDATE social_posts SET postType = 'text' WHERE postType IS NULL
    `);
    
    // Step 5: Update existing posts with imageUrl to have 'image' type and migrate the URL to mediaUrls
    console.log("Updating posts with images...");
    await sequelize.query(`
      UPDATE social_posts 
      SET postType = 'image',
          mediaUrls = JSON_ARRAY(imageUrl)
      WHERE imageUrl IS NOT NULL AND imageUrl != '' AND (mediaUrls IS NULL OR mediaUrls = '')
    `);
    
    console.log("Social post types migration completed successfully!");
  } catch (error) {
